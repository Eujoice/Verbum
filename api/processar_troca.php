...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_SESSION['usuario_matricula'])) {
    ...
    if (!preg_match($regex, $novaSenha)) {
        header("Location: ../pages/trocar_senha.php?erro=senha_fraca");
        exit();
    }

    // 2. Validação de Confirmação
    if ($novaSenha !== $confirmar) {
        header("Location: ../pages/trocar_senha.php?erro=confirmacao");
        exit();
    }
    ...
    if ($httpCode >= 200 && $httpCode < 300) {
        ...
        header("Location: ../pages/acervo.php?sucesso=senha_atualizada");
    } else {
        header("Location: ../pages/trocar_senha.php?erro=servidor");
    }
    exit();
} else {
