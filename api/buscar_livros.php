...
// Cria um mapa de empréstimos ativos [obra_id => { emprestimo_doc_id, matricula_usuario, data_prevista }]
$mapaEmprestimos = [];
    ...
    foreach ($dadosEmprestimos['documents'] as $emp) {
        ...
        if ($statusEmp === 'ativo') {
            ...
            $mapaEmprestimos[$idObra] = [
                'doc_id'            => $docId,
                'matricula_usuario' => $fields['usuario_id']['stringValue'] ?? '',
                'data_prevista'     => $fields['data_devolucao_prevista']['stringValue'] ?? '',
            ];
        }
    }
}
    ...
    foreach ($dadosObras['documents'] as $doc) {
        ...
        if (strpos(mb_strtolower($id), $termo) !== false || strpos(mb_strtolower($titulo), $termo) !== false) {
            ...
            $resultados[] = [
                ...
                'data_prevista'       => $emp['data_prevista'] ?? $data_prevista,
                'capa'                => $capa,
                'avaliacao'           => $avaliacao,
