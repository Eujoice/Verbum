...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($dadosFirebase['fields'])) {
        ...
        if ($emailDigitado === $emailNoBanco) {
            ...
            $link = "http://localhost/verbum/includes/recuperar_senha.php?token=$token&mat=$matricula";
            ...
            try {
                ...
                $mail->send();
                header("Location: ../pages/esqueceu_senha.php?sucesso=email_enviado");
            } catch (Exception $e) {
                // Se o e-mail falhar, avisa o SweetAlert
                header("Location: ../pages/esqueceu_senha.php?erro=envio_falhou");
            }
        } else {
            // E-mail não bate com a matrícula
            header("Location: ../pages/esqueceu_senha.php?erro=dados_invalidos");
        }
    } else {
        // Matrícula não existe no Firebase
        header("Location: ../pages/esqueceu_senha.php?erro=usuario_nao_encontrado");
    }
    exit();
}