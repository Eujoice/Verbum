...
async function carregarDados() {
    try {
        const response = await fetch('/verbum/api/listar_consulta.php');
...
function htmlCardExemplar(ex) {
    ...
    if (ex.avaliacao > 0) {
        ...
        for (let i = 1; i <= 5; i++) {
            if (nota >= i) {
                estrelas += '<span class="star star--cheia">★</span>';
            } else if (nota >= i - 0.5) {
                estrelas += '<span class="star star--meia">★</span>';
            } else {
                estrelas += '<span class="star star--vazia">★</span>';
            }
        }
    ...
    return `
    <a href="/verbum/pages/detalheslivro.php?id=${ex.id}" class="card-link-wrapper">
...
function htmlEmpStatsBar(filtrado) {
    ...
    <div class="emp-stats-bar">
        ...
        <button class="emp-stat-pill emp-stat-pill--export" onclick="exportarRelatorioEmpPDF()" title="Exportar relatório do filtro atual em PDF">
            <span class="emp-stat-num">
            ...
            <span class="emp-stat-label">Exportar PDF</span>
        </button>
    </div>`;
}
...
async function exportarRelatorioEmpPDF() {
    await Promise.all([
    ...
    // Monta a lista filtrada igual ao renderEmprestimos
    const filtradoBase = todosEmprestimos.filter(emp => {
        const ativo = emp.status !== 'inativo';
        const matchBusca = emp.aluno.toLowerCase().includes(termoBusca) ||
                           emp.livro.toLowerCase().includes(termoBusca);
        return ativo && matchBusca;
    });

    const lista = filtroEmpStatus === 'todos'
        ? filtradoBase
        : filtradoBase.filter(e => calcularStatusEmprestimo(e.devolucao).classeStatus === filtroEmpStatus);

    const configFiltro = {
        todos:    { label: 'Todos os Emprestimos Ativos', cor: [37, 99, 235],   corTexto: [255,255,255] },
        atrasado: { label: 'Emprestimos Atrasados',       cor: [220, 38, 38],   corTexto: [255,255,255] },
        hoje:     { label: 'Devolucoes para Hoje',        cor: [217, 119, 6],   corTexto: [255,255,255] },
        proximo:  { label: 'Vencem em Breve (3 dias)',    cor: [124, 58, 237],  corTexto: [255,255,255] },
        ok:       { label: 'Emprestimos Em Dia',          cor: [22, 163, 74],   corTexto: [255,255,255] }
    };
    const cfg = configFiltro[filtroEmpStatus] || configFiltro.todos;
    const dataHoje = new Date().toLocaleDateString('pt-BR');
    const nomeArquivo = 'emprestimos_' + filtroEmpStatus + '_' + new Date().toISOString().split('T')[0] + '.pdf';

    const doc = new jsPDF({ orientation: 'landscape' });
    const largura = doc.internal.pageSize.width;

    // Cabecalho colorido
    doc.setFillColor(...cfg.cor);
    doc.rect(0, 0, largura, 30, 'F');
    doc.setTextColor(...cfg.corTexto);
    doc.setFontSize(16);
    doc.setFont('helvetica', 'bold');
    doc.text('Biblioteca - ' + cfg.label, 14, 13);
    doc.setFontSize(9);
    doc.setFont('helvetica', 'normal');
    doc.text('Gerado em: ' + dataHoje, 14, 23);
    doc.text('Total de registros: ' + lista.length, 100, 23);
    if (termoBusca) doc.text('Busca aplicada: "' + termoBusca + '"', 180, 23);

    const linhas = lista.map(emp => {
        const obra = todosExemplares.find(ex => ex.id === emp.livro);
        const titulo = obra ? obra.titulo : emp.livro;
        const { textoStatus, diasAtraso } = calcularStatusEmprestimo(emp.devolucao);
        const diasStr = diasAtraso > 0 ? diasAtraso + (diasAtraso === 1 ? ' dia' : ' dias') : '-';
        return [
            emp.aluno,
            emp.matricula,
            titulo,
            '#' + emp.livro,
            formatarDataBR(emp.retirada),
            formatarDataBR(emp.devolucao),
            textoStatus,
            diasStr
        ];
    });
    ...
    doc.autoTable({
        startY: 36,
        head: [['Aluno', 'Matricula', 'Titulo', 'ID Exemplar', 'Retirada', 'Devolucao Prevista', 'Situacao', 'Dias Atraso']],
        body: linhas,
        theme: 'striped',
        headStyles: {
            fillColor: cfg.cor,
            textColor: cfg.corTexto,
            fontStyle: 'bold',
            fontSize: 8
        },
        ...
        willDrawCell: function(data) {
            ...
            const emp = lista[data.row.index];
            if (!emp) return;
            const { classeStatus } = calcularStatusEmprestimo(emp.devolucao);
            const cor = coresStatus[classeStatus];
            if (cor && filtroEmpStatus === 'todos') {
        ...
        didDrawPage: function(data) {
            ...
            doc.text(
                'Pagina ' + data.pageNumber + ' de ' + totalPags,
                largura - 30,
    ...
    // Rodape de resumo apenas no filtro "todos"
    if (filtroEmpStatus === 'todos' && lista.length > 0) {
        const atrasados = lista.filter(e => calcularStatusEmprestimo(e.devolucao).classeStatus === 'atrasado').length;
        const venceHoje = lista.filter(e => calcularStatusEmprestimo(e.devolucao).classeStatus === 'hoje').length;
        const emDia    = lista.filter(e => calcularStatusEmprestimo(e.devolucao).classeStatus === 'ok').length;
        const proximos = lista.filter(e => calcularStatusEmprestimo(e.devolucao).classeStatus === 'proximo').length;
        ...
        if (y < doc.internal.pageSize.height - 30) {
            ...
            doc.setFont('helvetica', 'normal');
            doc.setTextColor(220, 38, 38);  doc.text('Atrasados: ' + atrasados, 40, y);
            doc.setTextColor(217, 119, 6);  doc.text('Vencem hoje: ' + venceHoje, 90, y);
            doc.setTextColor(124, 58, 237); doc.text('Vencem em breve: ' + proximos, 145, y);
            doc.setTextColor(22, 163, 74);  doc.text('Em dia: ' + emDia, 215, y);
        }
    }
...
async function devolverLivro(idObra, idEmprestimo) {
    ...
    try {
        const response = await fetch('/verbum/api/processar_devolucao.php', {
            method: 'POST',
            body: formData
        });
...
async function enviarNotificacaoEmp(matricula, tituloLivro, idEmprestimo) {
    ...
    try {
        const response = await fetch('/verbum/api/processar_notificacao_manual.php', { method: 'POST', body: formData });
...
async function postReserva(formData, confirmacaoMsg) {
    ...
    try {
        const response = await fetch('/verbum/api/processar_reserva.php', { method: 'POST', body: formData });
        const resultado = await response.json();
...
async function exportarHistoricoCSV() {
    ...
    doc.setFontSize(16);
    doc.setFont('helvetica', 'bold');
    doc.text('Histórico de Empréstimos', 14, 12);
    doc.setFontSize(9);
    doc.setFont('helvetica', 'normal');
    doc.text(`Gerado em: ${dataHoje}`, 14, 22);
    doc.text(`Total de registros: ${lista.length}`, 120, 22);
    ...
    const linhas = lista.map(h => {
        ...
        const dur = (h.dataRetirada && h.dataDevolucao && h.dataDevolucao !== '—')
            ? diffDias(h.dataRetirada, h.dataDevolucao) + ' dias' : '—';
        return [
            h.aluno,
            h.livro,
            titulo,
    ...
    doc.autoTable({
        startY: 34,
        head: [['Aluno', 'ID Exemplar', 'Título', 'Data Retirada', 'Data Devolução', 'Dias']],
        body: linhas,
        theme: 'striped',
        headStyles: {
            fillColor: [37, 99, 235],
...
/* ── Sincronização Automática ──
   CORREÇÃO: Intervalo aumentado de 15s para 5 minutos (300000ms).
   A página do bibliotecário não precisa de sync a cada 15 segundos;
   isso gerava centenas de leituras desnecessárias no Firestore por turno.
   Ações manuais (devolver, emprestar) já chamam carregarDados() diretamente. */
...
async function notificarAluno(matricula, tituloLivro, idReserva) {
    ...
    try {
        const response = await fetch('/verbum/api/processar_reserva.php', {
            method: 'POST',
            body: formData
        });
...
async function buscarNotificacoesAluno() {
    try {
        ...
        const response = await fetch('/verbum/api/listar_notificacoes.php');
...
function abrirDetalheNotif(id) {
    ...
    // Marca como lida DEPOIS de abrir o painel
...
function fecharPainelDetalhe() {
    ...
    // Atualiza a lista ao voltar para refletir notificações lidas
    renderListaNotificacoes();
}
...
async function marcarComoLida(event, idNotificacao) {
    ...
    // Só re-renderiza a lista se o painel de detalhe NÃO estiver aberto
    // (evita fechar o painel ao clicar numa notificação)
    const painel = document.getElementById('notifPainelDetalhe');
    ...
    try {
        await fetch('/verbum/api/listar_notificacoes.php', { method: 'POST', body: formData });
    } catch (e) {
